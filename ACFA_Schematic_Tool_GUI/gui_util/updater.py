...
# --- Configuration ---
GITHUB_REPO = "tmbkoren/ACFA_Schematic_Tool"
API_URL = f"https://api.github.com/repos/{GITHUB_REPO}/releases/latest"
RELEASES_PAGE = f"https://github.com/{GITHUB_REPO}/releases/latest"

# The in-app auto-updater swaps a packaged .exe via a .bat and is Windows-only.
# On other platforms we only notify and point at the releases page.
...
def show_update_dialog(release_info, parent):
    ...
    release_url = release_info.get("html_url", RELEASES_PAGE)

    if not WINDOWS:
        # No auto-apply off Windows — notify and point at the download page.
        msg_box = QMessageBox(parent)
        msg_box.setIcon(QMessageBox.Icon.Information)
        msg_box.setWindowTitle("Update Available")
        msg_box.setText(f"A new version ({latest_version}) is available.")
        msg_box.setInformativeText(
            "The in-app updater is Windows-only. Download the latest release "
            f"manually:\n{release_url}\n\nRelease Notes:\n{release_notes}")
...
def create_and_run_updater_script(new_path, old_path):
    ...
    # CREATE_NEW_CONSOLE only exists on Windows; default to 0 elsewhere.
    create_new_console = getattr(subprocess, "CREATE_NEW_CONSOLE", 0)
    subprocess.Popen([script_path], creationflags=create_new_console)
